...
type Combo = {
  ...
  price: number; originalPrice: number; category: string; image: string; images?: string[];
  badge: string; badgeColor: string; stock: number; rating: number; reviews: number; care: string;
  isFeatured?: boolean; isBestseller?: boolean; isNewArrival?: boolean; isHidden?: boolean; saving?: number; festival?: string;
};
...
function OldAdminPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, startIndex?: number) => {
    ...
    const uploadPromises = validFiles.map((file, i) => {
      return new Promise<void>((resolve, reject) => {
        console.log(`[ProductUpload] Image ${i+1} selected: ${file.name}`);
        const tempUrl = tempUrls[i];
        const targetIndex = startIndex !== undefined ? startIndex + i : undefined;

        const img = new Image();
        img.onload = () => {
          console.log(`[ProductUpload] Image ${i+1} loaded. Starting compression...`);
          try {
            ...
            canvas.toBlob(async (blob) => {
              if (!blob) return reject(new Error("Empty blob generated by canvas"));
              ...
              try {
                ...
                console.log(`[ProductUpload] Image ${i+1} Firestore UI state update completed.`);
                
                completedCount++;
                toast.loading(`Uploading ${completedCount}/${validFiles.length} images...`, { id: toastId });
                resolve();
              } catch (err: any) {
                console.error(`[ProductUpload] Image ${i+1} Firebase upload error:`, err);
                toast.error(`Firebase Error: ${err.message || err.code || err}`);
                reject(err);
              }
            }, "image/jpeg", 0.7);
          } catch (err: any) {
            console.error(`[ProductUpload] Image ${i+1} canvas error:`, err);
            toast.error(`Canvas Error: ${err.message || err}`);
            reject(err);
          }
        };
        img.onerror = () => {
          console.error(`[ProductUpload] Image ${i+1} failed to load from local Blob URL.`);
          toast.error("Failed to load image preview.");
          reject(new Error("Image load failed"));
        };
        img.src = tempUrl;
      }).catch(err => {
        console.error(`[ProductUpload] Failed to process image ${i+1}:`, err);
        setFormData(prev => {
  ...
  const handleComboImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, startIndex?: number) => {
    ...
    const uploadPromises = validFiles.map((file, i) => {
      return new Promise<void>((resolve, reject) => {
        console.log(`[ComboUpload] Image ${i+1} selected: ${file.name}`);
        const tempUrl = tempUrls[i];
        const targetIndex = startIndex !== undefined ? startIndex + i : undefined;

        const img = new Image();
        img.onload = () => {
          console.log(`[ComboUpload] Image ${i+1} loaded. Starting compression...`);
          try {
            const canvas = document.createElement("canvas");
            const ctx = canvas.getContext("2d");
            if (!ctx) return reject(new Error("No ctx"));
            ...
            canvas.toBlob(async (blob) => {
              ...
              try {
                ...
                console.log(`[ComboUpload] Image ${i+1} Firestore UI state update completed.`);
                
                completedCount++;
                toast.loading(`Uploading ${completedCount}/${validFiles.length} images...`, { id: toastId });
                resolve();
              } catch (err: any) {
                console.error(`[ComboUpload] Image ${i+1} Firebase upload error:`, err);
                toast.error(`Firebase Error: ${err.message || err.code || err}`);
                reject(err);
              }
            }, "image/jpeg", 0.6);
          } catch (err: any) {
            console.error(`[ComboUpload] Image ${i+1} canvas error:`, err);
            toast.error(`Canvas Error: ${err.message || err}`);
            reject(err);
          }
        };
        img.onerror = () => {
          console.error(`[ComboUpload] Image ${i+1} failed to load from local Blob URL.`);
          toast.error("Failed to load image preview.");
          reject(new Error("Image load failed"));
        };
        img.src = tempUrl;
      }).catch(err => {
        console.error(`[ComboUpload] Failed to process image ${i+1}:`, err);
        setComboData(prev => {
          const newImages = [...(prev.images || [])].filter(img => img !== tempUrls[i]);
          return { ...prev, images: newImages, image: newImages[0] || "" };
        });
            ...
            {editingCombo ? (
              <div className="bg-white p-6 rounded-2xl shadow-sm border mb-8">
                ...
                <div className="grid grid-cols-1 gap-4">
                  ...
                  <div className="col-span-1 md:col-span-2">
                    <textarea placeholder="Care Instructions" rows={2} value={comboData.care || ''} onChange={e => setComboData({ ...comboData, care: e.target.value })} className="border p-2 rounded w-full" />
                  </div>
                  <div className="col-span-1 md:col-span-2 border p-4 rounded bg-gray-50 flex flex-col gap-4">
